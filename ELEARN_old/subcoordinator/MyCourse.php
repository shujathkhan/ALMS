...
$table1='coordinator_0_'.$id;    //role can either be 0 or 1 
$table2='coordinator_1_'.$id;
$sql="SELECT CONCAT( GROUP_CONCAT(table_name) , '' ) FROM information_schema.tables WHERE table_schema = '$department' AND table_name LIKE '$table1' OR table_name LIKE '$table2' ";
$result= mysqli_query($conn,$sql);
if($result){
while($row=mysqli_fetch_row($result)){
//$tablename='course';//$row[0];
$tablename=$row[0];
if(mysqli_num_rows(mysqli_query($conn,"SHOW TABLES LIKE '".$tablename."'"))>=1)   //check if table exist
{
$res=mysqli_query($conn,sprintf("SELECT DISTINCT courseid,coursename from $tablename"));	
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <form action="democoordinatorenrollfaculty.php" method="post" >
		  ...
		  $cid=$row[0];
$query1 = sprintf("SELECT firstname, staffid FROM overallfaculty where role='faculty'");
$result1 =mysqli_query($conn,$query1); 
...
<div id="coursecircle" class="col-sm-2" style="display:none;">
		...
		<label class="control-label" for="unit_1">Unit 1 : </label>
            <input type="text" class="form-control" name="unit_1" id="unit_1" title="Unit_1" value="'.$row['s1'].'" /> <br>
			<label class="control-label" for="unit_2">Unit 2 : </label>        
			<input type="text" class="form-control" name="unit_2" id="unit_2" title="Unit_2" value="'.$row['s2'].'"/> <br>
            <label class="control-label" for="unit_3">Unit 3 : </label>
		    <input type="text" class="form-control" name="unit_3" id="unit_3" title="Unit_3" value="'.$row['s3'].'"/> <br>
            <label class="control-label" for="unit_4">Unit 4 : </label>
			<input type="text" class="form-control" name="unit_4" id="unit_4" title="Unit_4" value="'.$row['s4'].'"/> <br>
            <label class="control-label" for="unit_5">Unit 5 : </label>
			<input type="text" class="form-control" name="unit_5" id="unit_5" title="Unit_5" value="'.$row['s5'].'"/> <br>
