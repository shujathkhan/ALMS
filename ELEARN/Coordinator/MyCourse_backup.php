...
$count+=1;
$select.='<option value="" selected disabled>Select Course</option>
<option value='.$row[0].' >'.$row[1].'-'.$row[0].'</option>';
	}
	$select.='</select><br>';
	$select.='<center><button type="submit" name="submit" class="waves-effect waves-light btn green" >Submit</button></center>
	</div></div>
	</form><br><br>';
	echo $select;
}else
...
$select.='<td><button class="waves-effect waves-light btn red accent-4 modal-trigger" data-toggle="modal" data-target="efmodal'.$count.'"  >Enroll Faculty</button>
</td>';
$select.='</tr>';
$select.='<div class="container">
  
  <!-- Modal -->
  <div class="modal" id="efmodal'.$count.'" role="dialog">
...
$select.='<div class="input-field col s5"><select id="choice" class="browser-default" name="staffid">';

while($row=mysqli_fetch_array($result1))
$select.='<option value="" selected disabled>Select Faculty</option>
<option value="'.$row[1].'">'.$row[1].'-'.$row[0].'</option>';
$select.='</select></div><br>';
$select.='<br><div class="col s5"><button type="submit" name="submit" class="waves-effect waves-light btn green" data-dismiss="efmodal">Enroll</button></div>';
}
