$('document').ready(function () {
    ...
    function generateOuterCirlceColourArray(units,count,statusCodes) {
        ...
        units.forEach(function (item) {
            ...
            var session = 1;
            for(var i = 0;i < item; i++) {
                var final_id = "S" + subjectID + "U" + unit + "S" + session;
            ...
            for(var i = 0;i < item; i++) {
                ...
                if(sessionID === c) {
                    if(units[unit-1] == session) {
                        ...
                        var final_id = subjectID + "U" + unit + "S" + session;
                        ...
                        window.location = "Session-Form.php?u=" + unit + "&s=" + session + "&sub=" + subjectID;
                    }
                    exit = true;
                }
                session++;
    ...
    function onInnerCirlceClickListner(unitID) {
        unitID++;
        var subjectID = parseInt($("#subject").val());
        window.location = "Unit.php?u=" + unitID + "&sub=" + subjectID;
