...
<div class="wrapper">
      ...
      <ul class="sidebar-menu">
        ...
        <li class="active treeview">
          ...
          <ul class="treeview-menu">
            <li class="active treeview"><a href="MyCourse.html"><i class="fa fa-tasks"></i> My Course</a></li>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="Report.html"><i class="fa fa-dashboard active"></i> Report</a></li>
...
$courses=sprintf("SELECT DISTINCT(courseid)FROM $usertable ");
$carray=mysqli_query($conn,$courses);
$cresult='';
while($row = mysqli_fetch_row($carray)){
//$cresult.= '"'.$row[0].'"'.',';
$cresult.= '"'.$row[0].'"'.',';
}
$cresult = substr($cresult, 0, -1);
$query = sprintf("SELECT courseid,coursename,coursetype,department FROM course WHERE courseid NOT IN ($check) AND courseid IN ($cresult)");
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
         ...
         <form class="form-horizontal" method="POST" action="enrollcourse.php"  name="AddCourse" onsubmit="return validateForm()">';
if($role=='student' ||$role=='Student' || $role=='students') 
	$table='overallstudent'; 
else 
	$table='overallfaculty';
		 $query1 = sprintf("SELECT firstname, staffid FROM $table WHERE username = '$username' ");
$result1 =mysqli_query($conn,$query1); 
$select.='<div class="form-group"><label class="control-label col-sm-4" for="department">Select Staff:</label><div class="col-sm-5"><select  class="form-control" id="choice"  name="faculty">';
while($row=mysqli_fetch_array($result1))
$select.='<option>'.$row[1].'-'.$row[0].'</option>';


$select.='</select></div></div><br>';
$select.='<br><div class="form-group"><label class="control-label col-sm-4" for="department">                       </label><div class="col-sm-5"><button type="submit" name="enroll" class="btn btn-default" data-dismiss="efmodal">Enroll</button></div></div>
        </div>
...
<?php 
$result = mysqli_query($conn,"SELECT courseid, courserefid FROM course WHERE courseid='$cresult' ");    //need the courseid of the course users clicking on //this is sending the whole array
if($result){
while($row=mysqli_fetch_row($result)){
$courserefid = $row[1];
$courseid=$row[0];
...
$out=array();
$out[0]=0;
$tab='master_course_'.$courseid;
for($i=1;$i<=5;$i++){
$unitid=$courserefid.'U'.$i;
//$ace =mysqli_query($conn,sprintf("SELECT sidcount FROM unit WHERE unitid LIKE '$unitid' AND courseid='$courseid'"));
$ace =mysqli_query($conn,sprintf("SELECT COUNT(sid) FROM $tab WHERE sid LIKE '".$unitid."%' AND courseid='$courseid'"));
$out[$i]=mysqli_fetch_row($ace);
}
}
 ?>
   
		<label class="control-label" for="unit_1">Unit 1 : </label>
            <input type="text" class="form-control" name="unit_1" id="unit_1" title="Unit_1" value="<?php echo $out[1]?> " /> <br>
			<label class="control-label" for="unit_2">Unit 2 : </label>        
			<input type="text" class="form-control" name="unit_2" id="unit_2" title="Unit_2" value="<?php echo $out[2]?> "/> <br>
            <label class="control-label" for="unit_3">Unit 3 : </label>
		    <input type="text" class="form-control" name="unit_3" id="unit_3" title="Unit_3" value="<?php echo $out[3]?> "/> <br>
            <label class="control-label" for="unit_4">Unit 4 : </label>
			<input type="text" class="form-control" name="unit_4" id="unit_4" title="Unit_4" value="<?php echo $out[4]?> "/> <br>
            <label class="control-label" for="unit_5">Unit 5 : </label>
			<input type="text" class="form-control" name="unit_5" id="unit_5" title="Unit_5" value="<?php echo $out[5]?> "/> <br>
        </form>
