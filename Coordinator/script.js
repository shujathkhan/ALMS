$('document').ready(function () {
    ...
    function init() {
        $.ajax({
            url: "dependencies/helper/crefid.php",
          method: "POST",
...
$.ajax({
                ...
                if(sessionID === c) {
                    if(units[unit-1] == session) {
                        ...
                        //mark it done
                 		...
                 		$.ajax({
                            url: "dependencies/helper/sid_create.php",
                            data: {
                                "u" : unit,
                                "s" : session,
                                "crefid" : crefid
                        ...
                        window.location = "Session-Form.php";
                    }
                    exit = true;
                }
                session++;
    ...
    function onInnerCirlceClickListner(unitID) {
        unitID++;
        var subjectID = crefid;
        window.location = "Unit.php?u=" + unitID + "&crefid=" + crefid;
